...
const Chats = () => {
    ...
    return (
        <div className="flex flex-col justify-center w-1/4 bg-gray-300 px-8 pt-8 pb-3">
            ...
            {globalData && (
                <div className={`h-20 text-white font-semibold flex space-x-5 py-2 px-3 mb-3 rounded-xl cursor-pointer hover:bg-gray-500 ${selectedChat?.chatType === 'public' && globalData.getGlobalGroup.id === selectedChat.chatData.id ? `bg-gray-500` : `bg-gray-700`}`} onClick={() => selectChat(globalData.getGlobalGroup, 'public')}>
