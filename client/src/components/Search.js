...
const Search = ({ placeholderText, searchText, onChange }) => {
    return (
        <div className="flex items-center px-5 h-16 bg-gray-700 rounded-xl my-3 select-none">
            ...
            <input value={searchText} onChange={ event => onChange(event.target.value)} className="w-full bg-gray-700 ml-5 rounded-md focus:outline-none text-white" placeholder={placeholderText} autoComplete="off" />
        </div>
    )
}
